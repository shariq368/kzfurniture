...
export default function App() {
  ...
  // Load collections from localStorage or fallback to default
  ...
  const [activeCategoryId, setActiveCategoryId] = useState<string>(COLLECTIONS[0].id);
  const activeCategory = collections.find((c: Category) => c.id === activeCategoryId) || collections[0];

  const [selectedGalleryImg, setSelectedGalleryImg] = useState(activeCategory.gallery[0] || '');
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  const [activePreviewImg, setActivePreviewImg] = useState<string | null>(null);

  // Modal states for adding image
  ...
  // Update selected gallery image when category changes or when its gallery changes
  useEffect(() => {
    if (activeCategory && activeCategory.gallery.length > 0) {
      if (!activeCategory.gallery.includes(selectedGalleryImg)) {
  ...
  const handleAddImage = (categoryId: string, imageSrc: string) => {
    if (!imageSrc.trim()) return;
    setCollections((prev: Category[]) => prev.map((c: Category) => {
      if (c.id === categoryId) {
        if (c.gallery.includes(imageSrc)) return c;
        const updatedGallery = [...c.gallery, imageSrc];
        return {
          ...c,
          gallery: updatedGallery,
          // Fallback if mainImage was empty
          mainImage: c.mainImage ? c.mainImage : imageSrc
        };
      }
      return c;
    }));
  ...
  const handleDeleteImage = (categoryId: string, imageSrc: string) => {
    setCollections((prev: Category[]) => prev.map((c: Category) => {
      if (c.id === categoryId) {
        const newGallery = c.gallery.filter((img: string) => img !== imageSrc);
        return {
          ...c,
          gallery: newGallery,
          mainImage: c.mainImage === imageSrc ? (newGallery[0] || '') : c.mainImage
        };
      }
      return c;
    }));
  ...
  const compressAndAddImage = (base64Str: string) => {
    ...
    img.onload = () => {
      ...
      if (ctx) {
        ...
        try {
          handleAddImage(activeCategoryId, compressedBase64);
        ...
        handleAddImage(activeCategoryId, base64Str);
        setShowAddModal(false);
        setNewImageUrl('');
        setFileError('');
      }
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
              ...
              <div className="flex flex-col space-y-2">
                {collections.map((category: Category) => (
                  <button
                    ...
                    className={`text-left py-4 px-6 border-l-2 text-sm tracking-widest uppercase transition-all duration-300 flex items-center justify-between cursor-pointer ${activeCategoryId === category.id
                  ...
                  >
                    ...
                    <ChevronRight className={`w-4 h-4 transition-transform ${activeCategoryId === category.id ? 'translate-x-1 text-gold-premium' : 'text-gray-600'
                      }`} />
                  </button>
                ))}
              </div>
              ...
              <div className="p-8 border border-white/5 bg-[#121212]/80 space-y-4 shadow-xl">
                ...
                <div className="pt-4 flex flex-wrap items-center gap-6">
                  ...
                  <button
                    ...
                    className="text-xs tracking-widest uppercase font-bold text-gold-premium hover:text-white transition-colors flex items-center gap-1.5 cursor-pointer"
                  >
                    Add Image <Plus className="w-3.5 h-3.5" />
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="lg:col-span-7 space-y-6">
              ...
              <div className="relative aspect-[16/10] bg-[#121212] overflow-hidden border border-white/10 group flex items-center justify-center">
                {selectedGalleryImg ? (
                  <>
                    <AnimatePresence mode="wait">
                      <motion.img
                        ...
                        className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-102 cursor-pointer"
                        alt={activeCategory.name}
                        onClick={() => setActivePreviewImg(selectedGalleryImg)}
                      />
                    </AnimatePresence>
                    <div className="absolute top-4 right-4 bg-black/60 px-3 py-1.5 text-[9px] tracking-widest uppercase text-gold-premium font-medium border border-gold-premium/30 pointer-events-none">
                      Interactive Catalog
                    </div>
                  </>
                ) : (
                  <div className="flex flex-col items-center justify-center gap-3 p-8 text-center text-gray-500">
                    <Sparkles className="w-8 h-8 text-gold-premium/40" />
                    <p className="text-xs uppercase tracking-wider font-bold">Empty Collection</p>
                    <p className="text-xs text-gray-muted/65 max-w-xs font-light">
                      No images are currently displayed in this showroom collection. Click "Add Photo" below to showcase your furniture.
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-4 gap-4">
                {activeCategory.gallery.map((img: string, index: number) => (
                  <div key={index} className="relative aspect-[4/3] group/thumb overflow-hidden border-2 transition-all duration-300">
                    <button
                      onClick={() => setSelectedGalleryImg(img)}
                      className={`w-full h-full bg-[#121212] ${selectedGalleryImg === img ? 'border-gold-premium scale-[0.98]' : 'border-white/5 hover:border-white/30'
                        }`}
                    >
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        if (confirm("Are you sure you want to remove this image from the showroom?")) {
                          handleDeleteImage(activeCategory.id, img);
                        }
                      }}
                      className="absolute top-2 right-2 p-1.5 bg-black/85 hover:bg-red-600/90 text-white rounded-full border border-white/15 opacity-0 group-hover/thumb:opacity-100 transition-opacity duration-300 shadow-md cursor-pointer"
                ...
                {/* Add image thumbnail trigger card */}
                <button
                  onClick={() => setShowAddModal(true)}
                  className="aspect-[4/3] border-2 border-dashed border-white/15 hover:border-gold-premium/40 bg-[#121212]/40 hover:bg-gold-premium/5 transition-all duration-300 flex flex-col items-center justify-center gap-1 cursor-pointer text-gray-500 hover:text-gold-premium group/add select-none"
                >
                  <Plus className="w-5 h-5 group-hover/add:scale-110 transition-transform duration-300" />
                  <span className="text-[10px] uppercase tracking-wider font-bold">Add Photo</span>
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="space-y-1">
                <h3 className="text-lg font-serif-luxury font-bold text-white uppercase tracking-wider">Add Photo</h3>
                <p className="text-xs text-gray-muted font-light leading-relaxed">
                  Upload a photo from your device, or paste a web URL to display in the <span className="text-gold-premium font-bold">{activeCategory.name}</span> collection.
                </p>
              </div>

              {/* Option A: File Upload */}
              <div className="space-y-3">
                <span className="text-[10px] uppercase tracking-wider text-gold-premium font-bold block">Method 1: Upload File</span>
                <label className="border border-dashed border-white/10 hover:border-gold-premium/40 bg-white/5 hover:bg-gold-premium/5 p-6 rounded-none flex flex-col items-center justify-center gap-2 cursor-pointer transition-all duration-300 group">
                  ...
                  <span className="text-xs text-gray-300 font-bold uppercase tracking-wider">Choose Image File</span>
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  <span className="text-[10px] uppercase tracking-wider text-gold-premium font-bold block">Method 2: Paste Web URL</span>
                  <input
                    type="text"
                ...
                <button
                  onClick={() => {
                    if (newImageUrl.trim()) {
                      handleAddImage(activeCategory.id, newImageUrl.trim());
                      setShowAddModal(false);
                      setNewImageUrl('');
                      setFileError('');
                    } else {
